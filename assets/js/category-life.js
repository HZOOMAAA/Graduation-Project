...
document.getElementById('lifeInsuranceForm').addEventListener('submit', function(e) {
    ...
    fetch('submit_life_application.php', {
        method: 'POST',
        body: formData
    })
    ...
    .then(data => {
        ...
        if(data.status === 'success') {
            showAppModal('success', 'Calculation Complete', data.message || 'Life policy simulated successfully.');
            this.reset();
        } else {
    ...
    .catch(error => {
        ...
        btnText.style.display = 'inline-block';
        btnLoader.style.display = 'none';
        showAppModal('error', 'Connection Error', 'Server error. Please verify your backend routing.');
    });
});